...
namespace Java.Util.Concurrent.Locks
{
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Returns the thread last set by  <c>setExclusiveOwnerThread </c> , or <c>null </c> if never set. This method does not otherwise impose any synchronization or <c>volatile </c> field accesses. </para>        
				/// </summary>
				/// <returns>
		...
		/// <summary>
		///  <para>A version of AbstractQueuedSynchronizer in which synchronization state is maintained as a <c>long </c> . This class has exactly the same structure, properties, and methods as <c>AbstractQueuedSynchronizer </c> with the exception that all state-related parameters and results are defined as <c>long </c> rather than <c>int </c> . This class may be useful when creating synchronizers such as multilevel locks and barriers that require 64 bits of state.</para> <para>See AbstractQueuedSynchronizer for usage notes and examples.</para> <para> <para>1.6 </para> <para>Doug Lea </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				///  <para>Creates a new <c>AbstractQueuedLongSynchronizer </c> instance with initial synchronization state of zero. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 4)]
				...
				/// <summary>
				///  <para>Atomically sets synchronization state to the given updated value if the current state value equals the expected value. This operation has memory semantics of a <c>volatile </c> read and write.</para> <para></para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				///  <para>Queries whether any threads are waiting on the given condition associated with this synchronizer. Note that because timeouts and interrupts may occur at any time, a <c>true </c> return does not guarantee that a future <c>signal </c> will awaken any threads. This method is designed primarily for use in monitoring of the system state.</para> <para></para>        
				/// </summary>
				/// <returns>
    ...
    "a/util/Collection<Ljava/lang/Thread;>;")]
				...
				/// <summary>
				///  <para>Returns the current value of synchronization state. This operation has memory semantics of a <c>volatile </c> read. </para>        
				/// </summary>
				/// <returns>
				...
				/// <summary>
				///  <para>Condition implementation for a AbstractQueuedLongSynchronizer serving as the basis of a Lock implementation.</para> <para>Method documentation for this class describes mechanics, not behavioral specifications from the point of view of Lock and Condition users. Exported versions of this class will in general need to be accompanied by documentation describing condition semantics that rely on those of the associated  <c>AbstractQueuedLongSynchronizer </c> .</para> <para>This class is Serializable, but all fields are transient, so deserialized conditions have no waiters.</para> <para> <para>1.6 </para></para>    
				/// </summary>
				/// <java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		///  <para>Provides a framework for implementing blocking locks and related synchronizers (semaphores, events, etc) that rely on first-in-first-out (FIFO) wait queues. This class is designed to be a useful basis for most kinds of synchronizers that rely on a single atomic <c>int </c> value to represent state. Subclasses must define the protected methods that change this state, and which define what that state means in terms of this object being acquired or released. Given these, the other methods in this class carry out all queuing and blocking mechanics. Subclasses can maintain other state fields, but only the atomically updated <c>int </c> value manipulated using methods getState, setState and compareAndSetState is tracked with respect to synchronization.</para> <para>Subclasses should be defined as non-public internal helper classes that are used to implement the synchronization properties of their enclosing class. Class  <c>AbstractQueuedSynchronizer </c> does not implement any synchronization interface. Instead it defines methods such as acquireInterruptibly that can be invoked as appropriate by concrete locks and related synchronizers to implement their public methods.</para> <para>This class supports either or both a default  <b>exclusive</b> mode and a  <b>shared</b> mode. When acquired in exclusive mode, attempted acquires by other threads cannot succeed. Shared mode acquires by multiple threads may (but need not) succeed. This class does not "understand" these differences except in the mechanical sense that when a shared mode acquire succeeds, the next waiting thread (if one exists) must also determine whether it can acquire as well. Threads waiting in the different modes share the same FIFO queue. Usually, implementation subclasses support only one of these modes, but both can come into play for example in a ReadWriteLock. Subclasses that support only exclusive or only shared modes need not define the methods supporting the unused mode.</para> <para>This class defines a nested ConditionObject class that can be used as a Condition implementation by subclasses supporting exclusive mode for which method isHeldExclusively reports whether synchronization is exclusively held with respect to the current thread, method release invoked with the current getState value fully releases this object, and acquire, given this saved state value, eventually restores this object to its previous acquired state. No  <c>AbstractQueuedSynchronizer </c> method otherwise creates such a condition, so if this constraint cannot be met, do not use it. The behavior of ConditionObject depends of course on the semantics of its synchronizer implementation.</para> <para>This class provides inspection, instrumentation, and monitoring methods for the internal queue, as well as similar methods for condition objects. These can be exported as desired into classes using an <c>AbstractQueuedSynchronizer </c> for their synchronization mechanics.</para> <para>Serialization of this class stores only the underlying atomic integer maintaining state, so deserialized objects have empty thread queues. Typical subclasses requiring serializability will define a <c>readObject </c> method that restores this to a known initial state upon deserialization.</para> <para> <h3>Usage</h3></para> <para>To use this class as the basis of a synchronizer, redefine the following methods, as applicable, by inspecting and/or modifying the synchronization state using getState, setState and/or compareAndSetState:</para> <para> <ul> <li> <para>tryAcquire </para></li> <li> <para>tryRelease </para></li> <li> <para>tryAcquireShared </para></li> <li> <para>tryReleaseShared </para></li> <li> <para>isHeldExclusively </para></li></ul></para> <para>Each of these methods by default throws UnsupportedOperationException. Implementations of these methods must be internally thread-safe, and should in general be short and not block. Defining these methods is the  <b>only</b> supported means of using this class. All other methods are declared  <c>final </c> because they cannot be independently varied.</para> <para>You may also find the inherited methods from AbstractOwnableSynchronizer useful to keep track of the thread owning an exclusive synchronizer. You are encouraged to use them  this enables monitoring and diagnostic tools to assist users in determining which threads hold locks.</para> <para>Even though this class is based on an internal FIFO queue, it does not automatically enforce FIFO acquisition policies. The core of exclusive synchronization takes the form:</para> <para> <pre>
		/// Acquire:
		///     while (!tryAcquire(arg)) {
		///         <b>enqueue thread if it is not already queued</b>;
		///         <b>possibly block current thread</b>;
		///     }</pre></para> <para> <pre>Release:
		///     if (tryRelease(arg))
		///         <b>unblock the first queued thread</b>;
		/// </pre></para> <para>(Shared mode is similar but may involve cascading signals.)</para> <para>Because checks in acquire are invoked before enqueuing, a newly acquiring thread may  <b>barge</b> ahead of others that are blocked and queued. However, you can, if desired, define <c>tryAcquire </c> and/or <c>tryAcquireShared </c> to disable barging by internally invoking one or more of the inspection methods. In particular, a strict FIFO lock can define  <c>tryAcquire </c> to immediately return <c>false </c> if getFirstQueuedThread does not return the current thread. A normally preferable non-strict fair version can immediately return  <c>false </c> only if hasQueuedThreads returns  <c>true </c> and <c>getFirstQueuedThread </c> is not the current thread; or equivalently, that <c>getFirstQueuedThread </c> is both non-null and not the current thread. Further variations are possible.</para> <para>Throughput and scalability are generally highest for the default barging (also known as  <b>greedy</b>,  <b>renouncement</b>, and  <b>convoy-avoidance</b>) strategy. While this is not guaranteed to be fair or starvation-free, earlier queued threads are allowed to recontend before later queued threads, and each recontention has an unbiased chance to succeed against incoming threads. Also, while acquires do not "spin" in the usual sense, they may perform multiple invocations of <c>tryAcquire </c> interspersed with other computations before blocking. This gives most of the benefits of spins when exclusive synchronization is only briefly held, without most of the liabilities when it isn't. If so desired, you can augment this by preceding calls to acquire methods with "fast-path" checks, possibly prechecking hasContended and/or hasQueuedThreads to only do so if the synchronizer is likely not to be contended.</para> <para>This class provides an efficient and scalable basis for synchronization in part by specializing its range of use to synchronizers that can rely on <c>int </c> state, acquire, and release parameters, and an internal FIFO wait queue. When this does not suffice, you can build synchronizers from a lower level using atomic classes, your own custom java.util.Queue classes, and LockSupport blocking support.</para> <para> <h3>Usage Examples</h3></para> <para>Here is a non-reentrant mutual exclusion lock class that uses the value zero to represent the unlocked state, and one to represent the locked state. While a non-reentrant lock does not strictly require recording of the current owner thread, this class does so anyway to make usage easier to monitor. It also supports conditions and exposes one of the instrumentation methods:</para> <para> <pre>class Mutex implements Lock, java.io.Serializable {
		/// 
		/// 
		/// 
		...
		/// } 
		/// 
		///  </pre></para> <para>Here is a latch class that is like a CountDownLatch except that it only requires a single <c>signal </c> to fire. Because a latch is non-exclusive, it uses the <c>shared </c> acquire and release methods.</para> <para> <pre>class BooleanLatch {
		/// 
		/// 
		/// 
		///   private static class Sync extends AbstractQueuedSynchronizer {
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				///  <para>Creates a new <c>AbstractQueuedSynchronizer </c> instance with initial synchronization state of zero. </para>        
				/// </summary>
				[Dot42.DexImport("<init>", "()V", AccessFlags = 4)]
    ...
    "il/Collection<Ljava/lang/Thread;>;")]
				...
				/// <summary>
				///  <para>Condition implementation for a AbstractQueuedSynchronizer serving as the basis of a Lock implementation.</para> <para>Method documentation for this class describes mechanics, not behavioral specifications from the point of view of Lock and Condition users. Exported versions of this class will in general need to be accompanied by documentation describing condition semantics that rely on those of the associated  <c>AbstractQueuedSynchronizer </c> .</para> <para>This class is Serializable, but all fields are transient, so deserialized conditions have no waiters. </para>    
				/// </summary>
				/// <java-name>
 ...
 /* scope: __dot42__ */ 
		...
		///   final Condition notEmpty =  <b>lock.newCondition(); </b></pre></para> <para> <pre>  final Object[] items = new Object[100];
		///   int putptr, takeptr, count;</pre></para> <para> <pre>  public void put(Object x) throws InterruptedException {
		///      <b>lock.lock();
		///     try {</b>
		...
		///       lock.unlock();
		///     }</b>
		///   }</pre></para> <para> <pre>  public Object take() throws InterruptedException {
		///      <b>lock.lock();
		///     try {</b>
		///       while (count == 0)
		///          <b>notEmpty.await();</b>
