...
namespace Java.Text
{
 ...
 /* scope: __dot42__ */ 
		...
		/// String[] fmts = {"Sun", "Mon", "Tue", "Wed", "Thur", "Fri", "Sat"};</pre></para> <para> <pre>double[] limits2 = {0, 1, ChoiceFormat.nextDouble(1)};
 ...
 /* scope: __dot42__ */ 
		...
		/// </pre></para> <para></para></blockquote></para> <para> <para>Collator </para> <para>RuleBasedCollator </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				[global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
				protected internal CollationKey() /* TypeBuilder.AddDefaultConstructor */ 
				{
				}
		...
		/// }
		/// </pre></para> <para></para></blockquote></para> <para> <para>RuleBasedCollator </para> <para>CollationKey </para></para>    
		/// </summary>
		/// <java-name>
		/// java/text/Collator
		/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Returns the currency. </para> <para> <c>null </c> is returned if <c>setInternationalCurrencySymbol() </c> has been previously called with a value that is not a valid ISO 4217 currency code. </para> <para> <para>setCurrency(Currency) </para> <para>setInternationalCurrencySymbol(String) </para></para>        
				/// </summary>
				/// <returns>
 ...
 /* scope: __dot42__ */ 
		...
		///         '  <b>QuotedPattern</b> '
		///          <b>UnquotedPattern</b>
		/// </pre></para> <para></para></blockquote></para> <para>Within a  <b>String</b>, <c>"''" </c> represents a single quote. A  <b>QuotedString</b> can contain arbitrary characters except single quotes; the surrounding single quotes are removed. An  <b>UnquotedString</b> can contain arbitrary characters except single quotes and left curly brackets. Thus, a string that should result in the formatted message "'{0}'" can be written as <c>"'''{'0}''" </c> or <c>"'''{0}'''" </c> . </para> <para>Within a  <b>SubformatPattern</b>, different rules apply. A  <b>QuotedPattern</b> can contain arbitrary characters except single quotes, but the surrounding single quotes are  <b>not</b> removed, so they may be interpreted by the subformat. For example, <c>"{1,number,$'#',##}" </c> will produce a number format with the hash-sign quoted, with a result such as: "$#31,45". An  <b>UnquotedPattern</b> can contain arbitrary characters except single quotes, but curly braces within it must be balanced. For example,  <c>"ab {0} de" </c> and <c>"ab ' </c> ' de"} are valid subformatpatterns, but@code "ab {0'}' de" @endcode  and@code "ab  de"} are not. </para> <para>The  <b>ArgumentIndex</b> value is a non-negative integer written using the digits '0' through '9', and represents an index into the  <c>arguments </c> array passed to the <c>format </c> methods or the result array returned by the <c>parse </c> methods. </para> <para>The  <b>FormatType</b> and  <b>FormatStyle</b> values are used to create a  <c>Format </c> instance for the format element. The following table shows how the values map to <c>Format </c> instances. Combinations not shown in the table are illegal. A  <b>SubformatPattern</b> must be a valid pattern string for the <c>Format </c> subclass used. </para> <para> <table> <row> <entry> <para>Format Type </para></entry> <entry> <para>Format Style </para></entry> <entry> <para>Subformat Created  </para></entry></row> <row> <entry> <para> <b>(none)</b> </para></entry> <entry> <para> <c>null </c>   </para></entry></row> <row> <entry> <para> <c>number </c>  </para></entry> <entry> <para> <b>(none)</b> </para></entry> <entry> <para> <c>NumberFormat.getInstance(getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>integer </c>  </para></entry> <entry> <para> <c>NumberFormat.getIntegerInstance(getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>currency </c>  </para></entry> <entry> <para> <c>NumberFormat.getCurrencyInstance(getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>percent </c>  </para></entry> <entry> <para> <c>NumberFormat.getPercentInstance(getLocale()) </c>   </para></entry></row> <row> <entry> <para> <b>SubformatPattern</b> </para></entry> <entry> <para> <c>new DecimalFormat(subformatPattern, new DecimalFormatSymbols(getLocale())) </c>   </para></entry></row> <row> <entry> <para> <c>date </c>  </para></entry> <entry> <para> <b>(none)</b> </para></entry> <entry> <para> <c>DateFormat.getDateInstance(DateFormat.DEFAULT, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>short </c>  </para></entry> <entry> <para> <c>DateFormat.getDateInstance(DateFormat.SHORT, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>medium </c>  </para></entry> <entry> <para> <c>DateFormat.getDateInstance(DateFormat.DEFAULT, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>long </c>  </para></entry> <entry> <para> <c>DateFormat.getDateInstance(DateFormat.LONG, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>full </c>  </para></entry> <entry> <para> <c>DateFormat.getDateInstance(DateFormat.FULL, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <b>SubformatPattern</b> </para></entry> <entry> <para> <c>new SimpleDateFormat(subformatPattern, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>time </c>  </para></entry> <entry> <para> <b>(none)</b> </para></entry> <entry> <para> <c>DateFormat.getTimeInstance(DateFormat.DEFAULT, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>short </c>  </para></entry> <entry> <para> <c>DateFormat.getTimeInstance(DateFormat.SHORT, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>medium </c>  </para></entry> <entry> <para> <c>DateFormat.getTimeInstance(DateFormat.DEFAULT, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>long </c>  </para></entry> <entry> <para> <c>DateFormat.getTimeInstance(DateFormat.LONG, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>full </c>  </para></entry> <entry> <para> <c>DateFormat.getTimeInstance(DateFormat.FULL, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <b>SubformatPattern</b> </para></entry> <entry> <para> <c>new SimpleDateFormat(subformatPattern, getLocale()) </c>   </para></entry></row> <row> <entry> <para> <c>choice </c>  </para></entry> <entry> <para> <b>SubformatPattern</b> </para></entry> <entry> <para> <c>new ChoiceFormat(subformatPattern) </c>   </para></entry></row></table></para> <para> <h4>Usage Information</h4></para> <para>Here are some examples of usage:  <blockquote> <para></para> <para> <pre>
		/// Object[] arguments = {
		...
		/// </pre></para> <para></para></blockquote></para> <para>Typically, the message format will come from resources, and the arguments will be dynamically set at runtime. </para> <para>Example 2:  <blockquote> <para></para> <para> <pre>
		...
		/// </pre></para> <para></para></blockquote></para> <para>For more sophisticated patterns, you can use a <c>ChoiceFormat </c> to get output such as:  <blockquote> <para></para> <para> <pre>
		...
		/// </pre></para> <para></para></blockquote>You can either do this programmatically, as in the above example, or by using a pattern (see ChoiceFormat for more information) as in:  <blockquote> <para></para> <para> <pre>
		/// form.applyPattern("There {0,choice,0#are no files|1#is one file|1&amp;lt;are {0,number,integer} files}.");
		/// </pre></para> <para></para></blockquote></para> <para> <b>Note:</b> As we see above, the string produced by a  <c>ChoiceFormat </c> in <c>MessageFormat </c> is treated specially; occurances of '{' are used to indicated subformats, and cause recursion. If you create both a <c>MessageFormat </c> and  <c>ChoiceFormat </c> programmatically (instead of using the string patterns), then be careful not to produce a format that recurses on itself, which will cause an infinite loop. </para> <para>When a single argument is parsed more than once in the string, the last match will be the final result of the parsing. For example:  <blockquote> <para> <pre>
		...
		/// </pre> </para></blockquote></para> <para>Likewise, parsing with a <c>MessageFormat </c> object using patterns containing multiple occurrences of the same argument would return the last match. For example:  <blockquote> <para> <pre>
		/// MessageFormat mf = new MessageFormat("{0}, {0}, {0}");
		/// String forParsing = "x, y, z";
		/// Object[] objs = mf.parse(forParsing, new ParsePosition(0));
		/// // result now equals {new String("z")}
		/// </pre> </para></blockquote> <h4>Synchronization</h4></para> <para>Message formats are not synchronized. It is recommended to create separate format instances for each thread. If multiple threads access a format concurrently, it must be synchronized externally.</para> <para> <para>java.util.Formatter </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// </pre></para> <para></para></blockquote></para> <para>Combining <c>Collator </c> s is as simple as concatenating strings. Here is an example that combines two <c>Collator </c> s from two different locales:  <blockquote> <para></para> <para> <pre>
		/// // Create an en_US Collator object
		/// RuleBasedCollator en_USCollator = (RuleBasedCollator)Collator
		///         .getInstance(new Locale("en", "US", ""));</pre></para> <para> <pre>// Create a da_DK Collator object
		/// RuleBasedCollator da_DKCollator = (RuleBasedCollator)Collator
		///         .getInstance(new Locale("da", "DK", ""));</pre></para> <para> <pre>// Combine the two collators
		/// // First, get the collation rules from en_USCollator
		/// String en_USRules = en_USCollator.getRules();</pre></para> <para> <pre>// Second, get the collation rules from da_DKCollator
		/// String da_DKRules = da_DKCollator.getRules();</pre></para> <para> <pre>RuleBasedCollator newCollator = new RuleBasedCollator(en_USRules + da_DKRules);
		/// // newCollator has the combined rules
		/// </pre></para> <para></para></blockquote></para> <para>The next example shows to make changes on an existing table to create a new  <c>Collator </c> object. For example, add <c>"&amp; C &lt; ch, cH, Ch, CH" </c> to the <c>en_USCollator </c> object to create your own:  <blockquote> <para></para> <para> <pre>
		/// // Create a new Collator object with additional rules
		/// String addRules = "&amp; C &lt; ch, cH, Ch, CH";</pre></para> <para> <pre>RuleBasedCollator myCollator = new RuleBasedCollator(en_USCollator + addRules);
		/// // myCollator contains the new rules
		/// </pre></para> <para></para></blockquote></para> <para>The following example demonstrates how to change the order of non-spacing accents:  <blockquote> <para></para> <para> <pre>
		/// // old rule
		/// String oldRules = "= \u00a8 ; \u00af ; \u00bf" + "&lt; a , A ; ae, AE ; \u00e6 , \u00c6"
		///         + "&lt; b , B &lt; c, C &lt; e, E &amp; C &lt; d, D";</pre></para> <para> <pre>// change the order of accent characters
		/// String addOn = "&amp; \u00bf ; \u00af ; \u00a8;";</pre></para> <para> <pre>RuleBasedCollator myCollator = new RuleBasedCollator(oldRules + addOn);
		/// </pre></para> <para></para></blockquote></para> <para>The last example shows how to put new primary ordering in before the default setting. For example, in the Japanese <c>Collator </c> , you can either sort English characters before or after Japanese characters:  <blockquote> <para></para> <para> <pre>
		/// // get en_US Collator rules
		/// RuleBasedCollator en_USCollator = (RuleBasedCollator)
		///     Collator.getInstance(Locale.US);</pre></para> <para> <pre>// add a few Japanese character to sort before English characters
		/// // suppose the last character before the first base letter 'a' in
		/// // the English collation rule is 
		/// String jaString = "&amp; \u30A2 , \u30FC &lt; \u30C8";</pre></para> <para> <pre>RuleBasedCollator myJapaneseCollator =
		///     new RuleBasedCollator(en_USCollator.getRules() + jaString);
		/// </pre></para> <para></para></blockquote></para>    
		/// </summary>
		/// <java-name>
