...
namespace Org.Xmlpull.V1
{
		...
		/// import java.io.StringReader;</pre></para> <para> <pre>import org.xmlpull.v1.XmlPullParser;
		/// import org.xmlpull.v1.;
		/// import org.xmlpull.v1.;</pre></para> <para> <pre>public class SimpleXmlPullApp
		/// {</pre></para> <para> <pre>    public static void main (String args[])
		///         throws XmlPullParserException, IOException
		///     {
		///         XmlPullParserFactory factory = XmlPullParserFactory.newInstance();
		///         factory.setNamespaceAware(true);
		///         XmlPullParser xpp = factory.newPullParser();</pre></para> <para> <pre>        xpp.( new StringReader ( "&amp;lt;foo&gt;Hello World!&amp;lt;/foo&gt;" ) );
		...
		/// End tag foo
		/// End document
		/// </pre></para> <para>For more details on API usage, please refer to the quick Introduction available at </para> <para> <para>XmlPullParserFactory </para> <para>defineEntityReplacementText </para> <para>getName </para> <para>getNamespace </para> <para>getText </para> <para>next </para> <para>nextToken </para> <para>setInput </para> <para>FEATURE_PROCESS_DOCDECL </para> <para>FEATURE_VALIDATION </para> <para>START_DOCUMENT </para> <para>START_TAG </para> <para>TEXT </para> <para>END_TAG </para> <para>END_DOCUMENT</para> <para> </para> <para> </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				///           // ...
				///      }
				/// </pre></para> <para> <para>getNamespacePrefix </para> <para>getNamespaceUri </para> <para>getNamespace() </para> <para>getNamespace(String) </para></para>        
				/// </summary>
				/// <java-name>
				/// getNamespaceCount
				/// </java-name>
				...
				///  }
				///  return null;
				/// </pre></para> <para> <b>Please note:</b> parser implementations may provide more efficient lookup, e.g. using a Hashtable. The 'xml' prefix is bound to "http://www.w3.org/XML/1998/namespace", as defined in the  specification. Analogous, the 'xmlns' prefix is resolved to </para> <para> <para>getNamespaceCount </para> <para>getNamespacePrefix </para> <para>getNamespaceUri </para></para>        
				/// </summary>
				/// <java-name>
				/// getNamespace
				/// </java-name>
				...
				/// <summary>
				///  <para>Returns the number of attributes of the current start tag, or -1 if the current event type is not START_TAG</para> <para> <para>getAttributeNamespace </para> <para>getAttributeName </para> <para>getAttributePrefix </para> <para>getAttributeValue </para></para>        
				/// </summary>
				/// <java-name>
				/// getAttributeCount
				/// </java-name>
				...
				/// <summary>
				///  <para>Returns the type of the current event (START_TAG, END_TAG, TEXT, etc.)</para> <para> <para>next() </para> <para>nextToken() </para></para>        
				/// </summary>
				/// <java-name>
				/// getEventType
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Signalize that parser is at the very beginning of the document and nothing was read yet. This event type can only be observed by calling getEvent() before the first call to next(), nextToken, or nextTag()).</para> <para> <para>next </para> <para>nextToken </para></para>        
				/// </summary>
				/// <java-name>
				/// START_DOCUMENT
				/// </java-name>
				...
				/// <summary>
				///  <para>Logical end of the xml document. Returned from getEventType, next() and nextToken() when the end of the input document has been reached. </para> <para> <b>NOTE:</b> subsequent calls to  or  may result in exception being thrown.</para> <para> <para>next </para> <para>nextToken </para></para>        
				/// </summary>
				/// <java-name>
				/// END_DOCUMENT
				/// </java-name>
				...
				/// <summary>
				///  <para>Returned from getEventType(), ,  when a start tag was read. The name of start tag is available from getName(), its namespace and prefix are available from getNamespace() and getPrefix() if . See getAttribute* methods to retrieve element attributes. See getNamespace* methods to retrieve newly declared namespaces.</para> <para> <para>next </para> <para>nextToken </para> <para>getName </para> <para>getPrefix </para> <para>getNamespace </para> <para>getAttributeCount </para> <para>getDepth </para> <para>getNamespaceCount </para> <para>getNamespace </para> <para>FEATURE_PROCESS_NAMESPACES </para></para>        
				/// </summary>
				/// <java-name>
				/// START_TAG
				/// </java-name>
				...
				/// <summary>
				///  <para>Returned from getEventType(), , or  when an end tag was read. The name of start tag is available from getName(), its namespace and prefix are available from getNamespace() and getPrefix().</para> <para> <para>next </para> <para>nextToken </para> <para>getName </para> <para>getPrefix </para> <para>getNamespace </para> <para>FEATURE_PROCESS_NAMESPACES </para></para>        
				/// </summary>
				/// <java-name>
				/// END_TAG
				/// </java-name>
				...
				/// <summary>
				///  <para>A CDATA sections was just read; this token is available only from calls to . A call to next() will accumulate various text events into a single event of type TEXT. The text contained in the CDATA section is available by calling getText().</para> <para> <para>nextToken </para> <para>getText </para></para>        
				/// </summary>
				/// <java-name>
				/// CDSECT
				/// </java-name>
				...
				/// <summary>
				///  <para>An XML processing instruction declaration was just read. This event type is available only via . getText() will return text that is inside the processing instruction. Calls to next() will skip processing instructions automatically.  <para>nextToken </para> <para>getText </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>An XML comment was just read. This event type is this token is available via  only; calls to next() will skip comments automatically. The content of the comment can be accessed using the getText() method.</para> <para> <para>nextToken </para> <para>getText </para></para>        
				/// </summary>
				/// <java-name>
				/// COMMENT
				/// </java-name>
				...
				/// <summary>
				///  <para>An XML document type declaration was just read. This token is available from  only. The unparsed text inside the doctype is available via the getText() method.</para> <para> <para>nextToken </para> <para>getText </para></para>        
				/// </summary>
				/// <java-name>
				/// DOCDECL
				/// </java-name>
				...
				/// <summary>
				///  <para>This feature determines whether the parser processes namespaces. As for all features, the default value is false. </para> <para> <b>NOTE:</b> The value can not be changed during parsing an must be set before parsing.</para> <para> <para>getFeature </para> <para>setFeature </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>This feature determines whether namespace attributes are exposed via the attribute access methods. Like all features, the default value is false. This feature cannot be changed during parsing.</para> <para> <para>getFeature </para> <para>setFeature </para></para>        
				/// </summary>
				/// <java-name>
