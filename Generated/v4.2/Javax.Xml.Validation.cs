...
{
		/// <summary>
		///  <para>Immutable in-memory representation of grammar.</para> <para>This object represents a set of constraints that can be checked/ enforced against an XML document.</para> <para>A Schema object is thread safe and applications are encouraged to share it across many parsers in many threads.</para> <para>A Schema object is immutable in the sense that it shouldn't change the set of constraints once it is created. In other words, if an application validates the same document twice against the same Schema, it must always produce the same result.</para> <para>A Schema object is usually created from SchemaFactory.</para> <para>Two kinds of validators can be created from a Schema object. One is Validator, which provides highly-level validation operations that cover typical use cases. The other is ValidatorHandler, which works on top of SAX for better modularity.</para> <para>This specification does not refine the java.lang.Object#equals(java.lang.Object) method. In other words, if you parse the same schema twice, you may still get  <code>!schemaA.equals(schemaB)</code>.</para> <para> <para> </para> <para></para> <title>Revision</title> <para>446598 </para>,  <title>Date</title> <para>2006-09-15 05:55:40 -0700 (Fri, 15 Sep 2006) </para> <para> </para> <para> </para> <para> </para> <para>1.5 </para></para>    
		/// </summary>
		/// <java-name>
