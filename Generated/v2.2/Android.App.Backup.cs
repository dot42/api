...
namespace Android.App.Backup
{
 ...
 /* scope: __dot42__ */ 
    ...
    "s/ParcelFileDescriptor;)V", AccessFlags = 1)]
		...
		///         int dataSize = data.getDataSize();</pre></para> <para> <pre>        if (key.equals(MY_BACKUP_KEY_ONE)) {
		///             // process this kind of record here
		///             byte[] buffer = new byte[dataSize];
		///             data.readEntityData(buffer, 0, dataSize); // reads the entire entity at once</pre></para> <para> <pre>            // now 'buffer' holds the raw data and can be processed however
		///             // the agent wishes
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		///  <para>Provides an java.io.InputStream-like interface for accessing an entity's data during a restore operation. Used by BackupHelper classes within the BackupAgentHelper mechanism. </para> <para>When BackupHelper.restoreEntity() is called, the current entity's header has already been read from the underlying BackupDataInput. The entity's key string and total data size are available through this class's getKey() and size() methods, respectively. </para> <para> <b>Note:</b> The caller should take care not to seek or close the underlying data source, nor read more than size() bytes from the stream.</para> <para> <para>BackupAgentHelper </para> <para>BackupHelper </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// static final String MY_STRING_KEY = "storedstring";</pre></para> <para> <pre>public void onBackup(ParcelFileDescriptor oldState, BackupDataOutput data, ParcelFileDescriptor newState)
		///         throws IOException {
		///     ...
 ...
 /* scope: __dot42__ */ 
		...
		/// import android.app.backup.SharedPreferencesBackupHelper;</pre></para> <para> <pre>public class MyBackupAgent extends BackupAgentHelper {
		///     // The names of the SharedPreferences groups that the application maintains.  These
		///     // are the same strings that are passed to Context#getSharedPreferences(String, int).
		///     static final String PREFS_DISPLAY = "displayprefs";
		///     static final String PREFS_SCORES = "highscores";</pre></para> <para> <pre>    // An arbitrary string used within the BackupAgentHelper implementation to
		///     // identify the SharedPreferenceBackupHelper's data.
		///     static final String MY_PREFS_BACKUP_KEY = "myprefs";</pre></para> <para> <pre>    // Simply allocate a helper and install it
		///     void onCreate() {
