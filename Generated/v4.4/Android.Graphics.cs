 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Modifies the bitmap to have a specified width, height, and Config, without affecting the underlying allocation backing the bitmap. Bitmap pixel data is not re-initialized for the new configuration.</para> <para>This method can be used to avoid allocating a new bitmap, instead reusing an existing bitmap's allocation for a new configuration of equal or lesser size. If the Bitmap's allocation isn't large enough to support the new configuration, an IllegalArgumentException will be thrown and the bitmap will not be modified.</para> <para>The result of getByteCount() will reflect the new configuration, while getAllocationByteCount() will reflect that of the initial configuration.</para> <para>WARNING: This method should NOT be called on a bitmap currently used by the view system. It does not make guarantees about how the underlying pixel buffer is remapped to the new config, just that the allocation is reused. Additionally, the view system does not account for bitmap properties being modifying during use, e.g. while attached to drawables.</para> <para> <para>setWidth(int) </para> <para>setHeight(int) </para> <para>setConfig(Config) </para></para>        
				/// </summary>
				/// <java-name>
				/// reconfigure
				/// </java-name>
				[Dot42.DexImport("reconfigure", "(IILandroid/graphics/Bitmap$Config;)V", AccessFlags = 1)]
				public void Reconfigure(int width, int height, global::Android.Graphics.Bitmap.Config config) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				///  <para>Convenience method for calling reconfigure(int, int, Config) with the current height and width.</para> <para>WARNING: this method should not be used on bitmaps currently used by the view system, see reconfigure(int, int, Config) for more details.</para> <para> <para>reconfigure(int, int, Config) </para> <para>setWidth(int) </para> <para>setHeight(int) </para></para>        
				/// </summary>
				/// <java-name>
				/// setConfig
				/// </java-name>
    ...
    "ics/Bitmap;", AccessFlags = 9)]
				...
				/// <summary>
				///  <para>Returns the size of the allocated memory used to store this bitmap's pixels.</para> <para>This can be larger than the result of getByteCount() if a bitmap is reused to decode other bitmaps of smaller size, or by manual reconfiguration. See reconfigure(int, int, Config), setWidth(int), setHeight(int), setConfig(Bitmap.Config), and BitmapFactory.Options.inBitmap. If a bitmap is not modified in this way, this value will be the same as that returned by getByteCount().</para> <para>This value will not change over the lifetime of a Bitmap.</para> <para> <para>reconfigure(int, int, Config) </para></para>        
				/// </summary>
				/// <java-name>
 ...
 /* scope: __dot42__ */ 
				{
						...
						/// <summary>
						///  <para>If true (which is the default), the resulting bitmap will have its color channels pre-multipled by the alpha channel.</para> <para>This should NOT be set to false for images to be directly drawn by the view system or through a Canvas. The view system and Canvas assume all drawn images are pre-multiplied to simplify draw-time blending, and will throw a RuntimeException when un-premultiplied are drawn.</para> <para>This is likely only useful if you want to manipulate raw encoded image data, e.g. with RenderScript or custom OpenGL.</para> <para>This does not affect bitmaps without an alpha channel.</para> <para> <para>Bitmap::hasAlpha() </para> <para>Bitmap::isPremultiplied() </para></para>        
						/// </summary>
						/// <java-name>
						/// inPremultiplied
						/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// getLocationZ
				/// </java-name>
				public virtual float LocationZ
				{
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Multi-plane Android YUV format</para> <para>This format is a generic YCbCr format, capable of describing any 4:2:0 chroma-subsampled planar or semiplanar buffer (but not fully interleaved), with 8 bits per color sample.</para> <para>Images in this format are always represented by three separate buffers of data, one for each color plane. Additional information always accompanies the buffers, describing the row stride and the pixel stride for each plane.</para> <para>The order of planes in the array returned by Image#getPlanes() is guaranteed such that plane #0 is always Y, plane #1 is always U (Cb), and plane #2 is always V (Cr).</para> <para>The Y-plane is guaranteed not to be interleaved with the U/V planes (in particular, pixel stride is always 1 in yPlane.getPixelStride()).</para> <para>The U/V planes are guaranteed to have the same row stride and pixel stride (in particular, uPlane.getRowStride() == vPlane.getRowStride() and uPlane.getPixelStride() == vPlane.getPixelStride(); ).</para> <para> <para>android.media.Image </para> <para>android.media.ImageReader </para> <para>android.hardware.camera2.CameraDevice </para></para>        
				/// </summary>
				/// <java-name>
				/// YUV_420_888
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				public NinePatch(global::Android.Graphics.Bitmap bitmap, sbyte[] chunk) /* MethodBuilder.Create */ 
				{
				}
				...
				public NinePatch(global::Android.Graphics.Bitmap bitmap, byte[] chunk) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				///  <para>Returns the name of this NinePatch object if one was specified when calling the constructor. </para>        
				/// </summary>
				/// <java-name>
				/// getName
				/// </java-name>
				...
				/// <summary>
				///  <para>Returns the paint used to draw this NinePatch. The paint can be null.</para> <para> <para>setPaint(Paint) </para> <para>draw(Canvas, Rect) </para> <para>draw(Canvas, RectF) </para></para>        
				/// </summary>
				/// <java-name>
				/// getPaint
				/// </java-name>
				...
				/// <summary>
				///  <para>Returns the bitmap used to draw this NinePatch. </para>        
				/// </summary>
				/// <java-name>
				/// getBitmap
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Paint flag that enables the use of bitmap fonts when drawing text.</para> <para>Disabling this flag will prevent text draw operations from using embedded bitmap strikes in fonts, causing fonts with both scalable outlines and bitmap strikes to draw only the scalable outlines, and fonts with only bitmap strikes to not draw at all.</para> <para> <para>Paint(int) </para> <para>setFlags(int) </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Font hinter option that disables font hinting.</para> <para> <para>setHinting(int) </para></para>        
				/// </summary>
				/// <java-name>
				/// HINTING_OFF
				/// </java-name>
				...
				/// <summary>
				///  <para>Font hinter option that enables font hinting.</para> <para> <para>setHinting(int) </para></para>        
				/// </summary>
				/// <java-name>
				/// HINTING_ON
				/// </java-name>
				...
				/// getTextLocale
				/// </java-name>
				public virtual global::Java.Util.Locale TextLocale
				{
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Set this path to the result of applying the Op to this path and the specified path. The resulting path will be constructed from non-overlapping contours. The curve order is reduced where possible so that cubics may be turned into quadratics, and quadratics maybe turned into lines.</para> <para> <para>Op </para> <para>op(Path, Path, android.graphics.Path.Op) </para></para>        
				/// </summary>
				/// <returns>
				///  <para>True if operation succeeded, false otherwise and this path remains unmodified.</para>
				/// </returns>
				/// <java-name>
				/// op
				/// </java-name>
				...
				/// <summary>
				///  <para>Set this path to the result of applying the Op to the two specified paths. The resulting path will be constructed from non-overlapping contours. The curve order is reduced where possible so that cubics may be turned into quadratics, and quadratics maybe turned into lines.</para> <para> <para>Op </para> <para>op(Path, android.graphics.Path.Op) </para></para>        
				/// </summary>
				/// <returns>
				///  <para>True if operation succeeded, false otherwise and this path remains unmodified.</para>
				/// </returns>
				...
				public virtual bool Op(global::Android.Graphics.Path path1, global::Android.Graphics.Path path2, global::Android.Graphics.Path.JavaOp op) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}
 ...
 /* scope: __dot42__ */ 
				{
						...
						/// values
						/// </java-name>
						[Dot42.DexImport("values", "()[Landroid/graphics/Path$Op;", AccessFlags = 9)]
						...
						/// valueOf
						/// </java-name>
						[Dot42.DexImport("valueOf", "(Ljava/lang/String;)Landroid/graphics/Path$Op;", AccessFlags = 9)]
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Construct a new SurfaceTexture to stream images to a given OpenGL texture.</para> <para>In single buffered mode the application is responsible for serializing access to the image content buffer. Each time the image content is to be updated, the releaseTexImage() method must be called before the image content producer takes ownership of the buffer. For example, when producing image content with the NDK ANativeWindow_lock and ANativeWindow_unlockAndPost functions, releaseTexImage() must be called before each ANativeWindow_lock, or that call will fail. When producing image content with OpenGL ES, releaseTexImage() must be called before the first OpenGL ES function call each frame.</para> <para></para>        
				/// </summary>
				[Dot42.DexImport("<init>", "(IZ)V", AccessFlags = 1)]
				public SurfaceTexture(int texName, bool singleBufferMode) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				///  <para>Releases the the texture content. This is needed in single buffered mode to allow the image content producer to take ownership of the image buffer. For more information see SurfaceTexture(int, boolean). </para>        
				/// </summary>
				/// <java-name>
				/// releaseTexImage
				/// </java-name>
