...
namespace Android.Content
{
 ...
 /* scope: __dot42__ */ 
    ...
    "g;)V", AccessFlags = 1)]
				...
				/// </java-name>
				public virtual bool IsLoadInBackgroundCanceled
				{
 ...
 /* scope: __dot42__ */ 
    ...
    "ang/String;)Landroid/database/Cursor;", AccessFlags = 1025)]
				...
				/// <summary>
				///  <para>Implement this to handle query requests from clients with support for cancellation. This method can be called from multiple threads, as described in . </para> <para>Example client call:</para> <para> <pre>// Request a specific record.
				/// Cursor managedCursor = managedQuery(
				///          ContentUris.withAppendedId(Contacts.People.CONTENT_URI, 2),
				///          projection,    // Which columns to return.
				///          null,          // WHERE clause.
    ...
    "ang/String;Landroid/os/CancellationSignal;)Landroid/database/Cursor;", AccessFlags = 1)]
				...
				/// <summary>
				///  <para>Implement this to support canonicalization of URIs that refer to your content provider. A canonical URI is one that can be transported across devices, backup/restore, and other contexts, and still be able to refer to the same data item. Typically this is implemented by adding query params to the URI allowing the content provider to verify that an incoming canonical URI references the same data as it was originally intended for and, if it doesn't, to find that data (if it exists) in the current environment.</para> <para>For example, if the content provider holds people and a normal URI in it is created with a row index into that people database, the cananical representation may have an additional query param at the end which specifies the name of the person it is intended for. Later calls into the provider with that URI will look up the row of that URI's base index and, if it doesn't match or its entry's name doesn't match the name in the query param, perform a query on its database to find the correct row to operate on.</para> <para>If you implement support for canonical URIs,  <b>all</b> incoming calls with URIs (including this one) must perform this verification and recovery of any canonical URIs they receive. In addition, you must also implement uncanonicalize to strip the canonicalization of any of these URIs.</para> <para>The default implementation of this method returns null, indicating that canonical URIs are not supported.</para> <para></para>        
				/// </summary>
				/// <returns>
				///  <para>Return the canonical representation of  <b>url</b>, or null if canonicalization of that Uri is not supported. </para>
				/// </returns>
				/// <java-name>
				/// canonicalize
				/// </java-name>
				[Dot42.DexImport("canonicalize", "(Landroid/net/Uri;)Landroid/net/Uri;", AccessFlags = 1)]
				public virtual global::Android.Net.Uri Canonicalize(global::Android.Net.Uri url) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Net.Uri);
				}

				/// <summary>
				///  <para>Remove canonicalization from canonical URIs previously returned by canonicalize. For example, if your implementation is to add a query param to canonicalize a URI, this method can simply trip any query params on the URI. The default implementation always returns the same  <b>url</b> that was passed in.</para> <para></para>        
				/// </summary>
				/// <returns>
				///  <para>Return the non-canonical representation of  <b>url</b>, return the  <b>url</b> as-is if there is nothing to do, or return null if the data identified by the canonical representation can not be found in the current environment. </para>
				/// </returns>
				/// <java-name>
				/// uncanonicalize
				/// </java-name>
				[Dot42.DexImport("uncanonicalize", "(Landroid/net/Uri;)Landroid/net/Uri;", AccessFlags = 1)]
				public virtual global::Android.Net.Uri Uncanonicalize(global::Android.Net.Uri url) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Net.Uri);
				}
    ...
    "arcelFileDescriptor;", AccessFlags = 1)]
				public virtual global::Android.OS.ParcelFileDescriptor OpenFile(global::Android.Net.Uri uri, string mode, global::Android.OS.CancellationSignal signal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.OS.ParcelFileDescriptor);
				}
    ...
    "ent/res/AssetFileDescriptor;", AccessFlags = 1)]
				public virtual global::Android.Content.Res.AssetFileDescriptor OpenAssetFile(global::Android.Net.Uri uri, string mode, global::Android.OS.CancellationSignal signal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Content.Res.AssetFileDescriptor);
				}
    ...
    "gnal;)Landroid/content/res/AssetFileDescriptor;", AccessFlags = 1)]
				public virtual global::Android.Content.Res.AssetFileDescriptor OpenTypedAssetFile(global::Android.Net.Uri uri, string mimeTypeFilter, global::Android.OS.Bundle opts, global::Android.OS.CancellationSignal signal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Content.Res.AssetFileDescriptor);
				}
    ...
    "ent/ContentProviderResult;")]
				...
				/// <summary>
				///  <para>Return the package name of the caller that initiated the request being processed on the current thread. The returned package will have been verified to belong to the calling UID. Returns <c>null </c> if not currently processing a request. </para> <para>This will always return <c>null </c> when processing getType(Uri) or getStreamTypes(Uri, String) requests.</para> <para> <para>Binder::getCallingUid() </para> <para>Context::grantUriPermission(String, Uri, int) </para></para>        
				/// </summary>
				/// <java-name>
				/// getCallingPackage
				/// </java-name>
 ...
 /* scope: __dot42__ */ 
    ...
    "ang/String;Landroid/os/CancellationSignal;)Landroid/database/Cursor;", AccessFlags = 1)]
				...
				/// <summary>
				///  <para>See ContentProvider#canonicalize </para>        
				/// </summary>
				/// <java-name>
				/// canonicalize
				/// </java-name>
				[Dot42.DexImport("canonicalize", "(Landroid/net/Uri;)Landroid/net/Uri;", AccessFlags = 17)]
				public global::Android.Net.Uri Canonicalize(global::Android.Net.Uri url) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Net.Uri);
				}

				/// <summary>
				///  <para>See ContentProvider#uncanonicalize </para>        
				/// </summary>
				/// <java-name>
				/// uncanonicalize
				/// </java-name>
				[Dot42.DexImport("uncanonicalize", "(Landroid/net/Uri;)Landroid/net/Uri;", AccessFlags = 17)]
				public global::Android.Net.Uri Uncanonicalize(global::Android.Net.Uri url) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Net.Uri);
				}
    ...
    "ring;)I", AccessFlags = 1)]
				...
				[Dot42.DexImport("openFile", "(Landroid/net/Uri;Ljava/lang/String;Landroid/os/CancellationSignal;)Landroid/os/P" +
    "arcelFileDescriptor;", AccessFlags = 1)]
				public virtual global::Android.OS.ParcelFileDescriptor OpenFile(global::Android.Net.Uri url, string mode, global::Android.OS.CancellationSignal signal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.OS.ParcelFileDescriptor);
				}
				...
				[Dot42.DexImport("openAssetFile", "(Landroid/net/Uri;Ljava/lang/String;Landroid/os/CancellationSignal;)Landroid/cont" +
    "ent/res/AssetFileDescriptor;", AccessFlags = 1)]
				public virtual global::Android.Content.Res.AssetFileDescriptor OpenAssetFile(global::Android.Net.Uri url, string mode, global::Android.OS.CancellationSignal signal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Content.Res.AssetFileDescriptor);
				}
    ...
    "gnal;)Landroid/content/res/AssetFileDescriptor;", AccessFlags = 17)]
				public global::Android.Content.Res.AssetFileDescriptor OpenTypedAssetFileDescriptor(global::Android.Net.Uri uri, string mimeType, global::Android.OS.Bundle opts, global::Android.OS.CancellationSignal signal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Content.Res.AssetFileDescriptor);
				}
 ...
 /* scope: __dot42__ */ 
    ...
    "ang/String;Landroid/os/CancellationSignal;)Landroid/database/Cursor;", AccessFlags = 17)]
				...
				/// <summary>
				///  <para>Transform the given  <b>url</b> to a canonical representation of its referenced resource, which can be used across devices, persisted, backed up and restored, etc. The returned Uri is still a fully capable Uri for use with its content provider, allowing you to do all of the same content provider operations as with the original Uri  query, openInputStream(android.net.Uri), etc. The only difference in behavior between the original and new Uris is that the content provider may need to do some additional work at each call using it to resolve it to the correct resource, especially if the canonical Uri has been moved to a different environment.</para> <para>If you are moving a canonical Uri between environments, you should perform another call to canonicalize with that original Uri to re-canonicalize it for the current environment. Alternatively, you may want to use uncanonicalize to transform it to a non-canonical Uri that works only in the current environment but potentially more efficiently than the canonical representation.</para> <para> <para>uncanonicalize </para></para>        
				/// </summary>
				/// <returns>
				///  <para>Returns the official canonical representation of  <b>url</b>, or null if the content provider does not support a canonical representation of the given Uri. Many providers may not support canonicalization of some or all of their Uris.</para>
				/// </returns>
				/// <java-name>
				/// canonicalize
				/// </java-name>
				...
				/// <summary>
				///  <para>Given a canonical Uri previously generated by canonicalize, convert it to its local non-canonical form. This can be useful in some cases where you know that you will only be using the Uri in the current environment and want to avoid any possible overhead when using it with the content provider or want to verify that the referenced data exists at all in the new environment.</para> <para> <para>canonicalize </para></para>        
				/// </summary>
				/// <returns>
				///  <para>Returns the non-canonical representation of  <b>url</b>. This will return null if data identified by the canonical Uri can not be found in the current environment; callers must always check for null and deal with that by appropriately falling back to an alternative.</para>
				/// </returns>
				/// <java-name>
				/// uncanonicalize
				/// </java-name>
    ...
    "arcelFileDescriptor;", AccessFlags = 17)]
				public global::Android.OS.ParcelFileDescriptor OpenFileDescriptor(global::Android.Net.Uri uri, string mode, global::Android.OS.CancellationSignal cancellationSignal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.OS.ParcelFileDescriptor);
				}
    ...
    "ent/res/AssetFileDescriptor;", AccessFlags = 17)]
				public global::Android.Content.Res.AssetFileDescriptor OpenAssetFileDescriptor(global::Android.Net.Uri uri, string mode, global::Android.OS.CancellationSignal cancellationSignal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Content.Res.AssetFileDescriptor);
				}
    ...
    "tFileDescriptor;", AccessFlags = 17)]
				...
				[Dot42.DexImport("openTypedAssetFileDescriptor", "(Landroid/net/Uri;Ljava/lang/String;Landroid/os/Bundle;Landroid/os/CancellationSi" +
    "gnal;)Landroid/content/res/AssetFileDescriptor;", AccessFlags = 17)]
				public global::Android.Content.Res.AssetFileDescriptor OpenTypedAssetFileDescriptor(global::Android.Net.Uri uri, string mimeType, global::Android.OS.Bundle opts, global::Android.OS.CancellationSignal cancellationSignal) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Content.Res.AssetFileDescriptor);
				}
    ...
    "d/os/Bundle;", AccessFlags = 17)]
				...
				/// <summary>
				///  <para>Take a persistable URI permission grant that has been offered. Once taken, the permission grant will be remembered across device reboots. Only URI permissions granted with Intent#FLAG_GRANT_PERSISTABLE_URI_PERMISSION can be persisted. If the grant has already been persisted, taking it again will touch UriPermission#getPersistedTime().</para> <para> <para>getPersistedUriPermissions() </para></para>        
				/// </summary>
				/// <java-name>
				/// takePersistableUriPermission
				/// </java-name>
				[Dot42.DexImport("takePersistableUriPermission", "(Landroid/net/Uri;I)V", AccessFlags = 1)]
				public virtual void TakePersistableUriPermission(global::Android.Net.Uri uri, int modeFlags) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				///  <para>Relinquish a persisted URI permission grant. The URI must have been previously made persistent with takePersistableUriPermission(Uri, int). Any non-persistent grants to the calling package will remain intact.</para> <para> <para>getPersistedUriPermissions() </para></para>        
				/// </summary>
				/// <java-name>
				/// releasePersistableUriPermission
				/// </java-name>
				[Dot42.DexImport("releasePersistableUriPermission", "(Landroid/net/Uri;I)V", AccessFlags = 1)]
				public virtual void ReleasePersistableUriPermission(global::Android.Net.Uri uri, int modeFlags) /* MethodBuilder.Create */ 
				{
				}
				...
				/// <summary>
				///  <para>Register a sync with the SyncManager. These requests are built using the SyncRequest.Builder.</para> <para></para>        
				/// </summary>
				/// <java-name>
				/// requestSync
				/// </java-name>
				[Dot42.DexImport("requestSync", "(Landroid/content/SyncRequest;)V", AccessFlags = 9)]
				public static void RequestSync(global::Android.Content.SyncRequest request) /* MethodBuilder.Create */ 
				{
				}
    ...
    "riodicSync;>;")]
				...
				/// <summary>
				///  <para>Return list of all URI permission grants that have been persisted by the calling app. That is, the returned permissions have been granted  <b>to</b> the calling app. Only persistable grants taken with takePersistableUriPermission(Uri, int) are returned.</para> <para> <para>takePersistableUriPermission(Uri, int) </para> <para>releasePersistableUriPermission(Uri, int) </para></para>        
				/// </summary>
				/// <java-name>
				/// getPersistedUriPermissions
				/// </java-name>
				public virtual global::Java.Util.IList<global::Android.Content.UriPermission> PersistedUriPermissions
				{
						...
						get{ return default(global::Java.Util.IList<global::Android.Content.UriPermission>); }
				}

				/// <summary>
				///  <para>Return list of all persisted URI permission grants that are hosted by the calling app. That is, the returned permissions have been granted  <b>from</b> the calling app. Only grants taken with takePersistableUriPermission(Uri, int) are returned. </para>        
				/// </summary>
				/// <java-name>
				/// getOutgoingPersistedUriPermissions
				/// </java-name>
				public virtual global::Java.Util.IList<global::Android.Content.UriPermission> OutgoingPersistedUriPermissions
				{
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Database open flag: when set, the database is opened with write-ahead logging enabled by default.</para> <para> <para>openOrCreateDatabase(String, int, CursorFactory) </para> <para>openOrCreateDatabase(String, int, CursorFactory, DatabaseErrorHandler) </para> <para>SQLiteDatabase::enableWriteAheadLogging </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.view.accessibility.CaptioningManager for obtaining captioning properties and listening for changes in captioning preferences.</para> <para> <para>getSystemService </para> <para>android.view.accessibility.CaptioningManager </para></para>        
				/// </summary>
				/// <java-name>
				/// CAPTIONING_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.net.wifi.p2p.WifiP2pManager for handling management of Wi-Fi peer-to-peer connections.</para> <para> <para>getSystemService </para> <para>android.net.wifi.p2p.WifiP2pManager </para></para>        
				/// </summary>
				/// <java-name>
				/// WIFI_P2P_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.net.nsd.NsdManager for handling management of network service discovery</para> <para> <para>getSystemService </para> <para>android.net.nsd.NsdManager </para></para>        
				/// </summary>
				/// <java-name>
				/// NSD_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.media.MediaRouter for controlling and managing routing of media.</para> <para> <para>getSystemService </para> <para>android.media.MediaRouter </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.hardware.input.InputManager for interacting with input devices.</para> <para> <para>getSystemService </para> <para>android.hardware.input.InputManager </para></para>        
				/// </summary>
				/// <java-name>
				/// INPUT_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.hardware.display.DisplayManager for interacting with display devices.</para> <para> <para>getSystemService </para> <para>android.hardware.display.DisplayManager </para></para>        
				/// </summary>
				/// <java-name>
				/// DISPLAY_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.os.UserManager for managing users on devices that support multiple users.</para> <para> <para>getSystemService </para> <para>android.os.UserManager </para></para>        
				/// </summary>
				/// <java-name>
				/// USER_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.app.AppOpsManager for tracking application operations on the device.</para> <para> <para>getSystemService </para> <para>android.app.AppOpsManager </para></para>        
				/// </summary>
				/// <java-name>
				/// APP_OPS_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>android.print.PrintManager for printing and managing printers and print tasks.</para> <para> <para>getSystemService </para> <para>android.print.PrintManager </para></para>        
				/// </summary>
				/// <java-name>
				/// PRINT_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Use with getSystemService to retrieve a android.hardware.ConsumerIrManager for transmitting infrared signals from the device.</para> <para> <para>getSystemService </para> <para>android.hardware.ConsumerIrManager </para></para>        
				/// </summary>
				/// <java-name>
				/// CONSUMER_IR_SERVICE
				/// </java-name>
				...
				/// <summary>
				///  <para>Remove a ComponentCallbacks object that was previously registered with registerComponentCallbacks(ComponentCallbacks). </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Returns absolute paths to application-specific directories on all external storage devices where the application can place persistent files it owns. These files are internal to the application, and not typically visible to the user as media. </para> <para>This is like getFilesDir() in that these files will be deleted when the application is uninstalled, however there are some important differences:  <ul> <li> <para>External files are not always available: they will disappear if the user mounts the external storage on a computer or removes it. </para></li> <li> <para>There is no security enforced with these files. </para></li></ul></para> <para>External storage devices returned here are considered a permanent part of the device, including both emulated external storage and physical media slots, such as SD cards in a battery compartment. The returned paths do not include transient devices, such as USB flash drives. </para> <para>An application may store data on any or all of the returned devices. For example, an app may choose to store large files on the device with the most available space, as measured by StatFs. </para> <para>No permissions are required to read or write to the returned paths; they are always accessible to the calling app. Write access outside of these paths on secondary external storage devices is not available. </para> <para>The first path returned is the same as getExternalFilesDir(String). Returned paths may be <c>null </c> if a storage device is unavailable.</para> <para> <para>getExternalFilesDir(String) </para> <para>Environment::getStorageState(File) </para></para>        
				/// </summary>
				/// <java-name>
				/// getExternalFilesDirs
				/// </java-name>
				[Dot42.DexImport("getExternalFilesDirs", "(Ljava/lang/String;)[Ljava/io/File;", AccessFlags = 1025)]
				public abstract global::Java.IO.File[] GetExternalFilesDirs(string type) /* MethodBuilder.Create */ ;
    ...
    "ng;Landroid/os/Handler;)Landroid/content/Intent;", AccessFlags = 1025)]
				...
				public abstract global::Android.Content.Context CreateDisplayContext(global::Android.Views.Display display) /* MethodBuilder.Create */ ;
				...
				/// <summary>
				///  <para>Returns absolute paths to application-specific directories on all external storage devices where the application's OBB files (if there are any) can be found. Note if the application does not have any OBB files, these directories may not exist. </para> <para>This is like getFilesDir() in that these files will be deleted when the application is uninstalled, however there are some important differences:  <ul> <li> <para>External files are not always available: they will disappear if the user mounts the external storage on a computer or removes it. </para></li> <li> <para>There is no security enforced with these files. </para></li></ul></para> <para>External storage devices returned here are considered a permanent part of the device, including both emulated external storage and physical media slots, such as SD cards in a battery compartment. The returned paths do not include transient devices, such as USB flash drives. </para> <para>An application may store data on any or all of the returned devices. For example, an app may choose to store large files on the device with the most available space, as measured by StatFs. </para> <para>No permissions are required to read or write to the returned paths; they are always accessible to the calling app. Write access outside of these paths on secondary external storage devices is not available. </para> <para>The first path returned is the same as getObbDir(). Returned paths may be <c>null </c> if a storage device is unavailable.</para> <para> <para>getObbDir() </para> <para>Environment::getStorageState(File) </para></para>        
				/// </summary>
				/// <java-name>
				/// getObbDirs
				/// </java-name>
				public abstract global::Java.IO.File[] ObbDirs
				{
				...
				/// <summary>
				///  <para>Returns absolute paths to application-specific directories on all external storage devices where the application can place cache files it owns. These files are internal to the application, and not typically visible to the user as media. </para> <para>This is like getCacheDir() in that these files will be deleted when the application is uninstalled, however there are some important differences:  <ul> <li> <para>External files are not always available: they will disappear if the user mounts the external storage on a computer or removes it. </para></li> <li> <para>There is no security enforced with these files. </para></li></ul></para> <para>External storage devices returned here are considered a permanent part of the device, including both emulated external storage and physical media slots, such as SD cards in a battery compartment. The returned paths do not include transient devices, such as USB flash drives. </para> <para>An application may store data on any or all of the returned devices. For example, an app may choose to store large files on the device with the most available space, as measured by StatFs. </para> <para>No permissions are required to read or write to the returned paths; they are always accessible to the calling app. Write access outside of these paths on secondary external storage devices is not available. </para> <para>The first path returned is the same as getExternalCacheDir(). Returned paths may be <c>null </c> if a storage device is unavailable.</para> <para> <para>getExternalCacheDir() </para> <para>Environment::getStorageState(File) </para></para>        
				/// </summary>
				/// <java-name>
				/// getExternalCacheDirs
				/// </java-name>
				public abstract global::Java.IO.File[] ExternalCacheDirs
				{
 ...
 /* scope: __dot42__ */ 
		{
				...
				public override global::Java.IO.File[] GetExternalFilesDirs(string type) /* MethodBuilder.Create */ 
				{
						return default(global::Java.IO.File[]);
				}
    ...
    "ng;Landroid/os/Handler;)Landroid/content/Intent;", AccessFlags = 1)]
				...
				public override global::Android.Content.Context CreateDisplayContext(global::Android.Views.Display display) /* MethodBuilder.Create */ 
				{
						return default(global::Android.Content.Context);
				}
				...
				/// getObbDirs
				/// </java-name>
				public override global::Java.IO.File[] ObbDirs
				{
				...
				/// getExternalCacheDirs
				/// </java-name>
				public override global::Java.IO.File[] ExternalCacheDirs
				{
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		///  <para>Extended ComponentCallbacks interface with a new callback for finer-grained memory management. This interface is available in all application components (android.app.Activity, android.app.Service, ContentProvider, and android.app.Application).</para> <para>You should implement onTrimMemory to incrementally release memory based on current system constraints. Using this callback to release your resources helps provide a more responsive system overall, but also directly benefits the user experience for your app by allowing the system to keep your process alive longer. That is, if you  <b>don't</b> trim your resources based on memory levels defined by this callback, the system is more likely to kill your process while it is cached in the least-recently used (LRU) list, thus requiring your app to restart and restore all state when the user returns to it.</para> <para>The values provided by onTrimMemory do not represent a single linear progression of memory limits, but provide you different types of clues about memory availability:</para> <para> <ul> <li> <para>When your app is running:  <ol> <li> <para>TRIM_MEMORY_RUNNING_MODERATE  <br></br>The device is beginning to run low on memory. Your app is running and not killable. </para></li> <li> <para>TRIM_MEMORY_RUNNING_LOW  <br></br>The device is running much lower on memory. Your app is running and not killable, but please release unused resources to improve system performance (which directly impacts your app's performance). </para></li> <li> <para>TRIM_MEMORY_RUNNING_CRITICAL  <br></br>The device is running extremely low on memory. Your app is not yet considered a killable process, but the system will begin killing background processes if apps do not release resources, so you should release non-critical resources now to prevent performance degradation. </para></li></ol></para></li> <li> <para>When your app's visibility changes:  <ol> <li> <para>TRIM_MEMORY_UI_HIDDEN  <br></br>Your app's UI is no longer visible, so this is a good time to release large resources that are used only by your UI. </para></li></ol></para></li> <li> <para>When your app's process resides in the background LRU list:  <ol> <li> <para>TRIM_MEMORY_BACKGROUND  <br></br>The system is running low on memory and your process is near the beginning of the LRU list. Although your app process is not at a high risk of being killed, the system may already be killing processes in the LRU list, so you should release resources that are easy to recover so your process will remain in the list and resume quickly when the user returns to your app. </para></li> <li> <para>TRIM_MEMORY_MODERATE  <br></br>The system is running low on memory and your process is near the middle of the LRU list. If the system becomes further constrained for memory, there's a chance your process will be killed. </para></li> <li> <para>TRIM_MEMORY_COMPLETE  <br></br>The system is running low on memory and your process is one of the first to be killed if the system does not recover memory now. You should release absolutely everything that's not critical to resuming your app state. </para> <para>To support API levels lower than 14, you can use the onLowMemory method as a fallback that's roughly equivalent to the ComponentCallbacks2#TRIM_MEMORY_COMPLETE level.  </para></li></ol></para> <para> <b>Note:</b> When the system begins killing processes in the LRU list, although it primarily works bottom-up, it does give some consideration to which processes are consuming more memory and will thus provide more gains in memory if killed. So the less memory you consume while in the LRU list overall, the better your chances are to remain in the list and be able to quickly resume.</para> <para></para></li></ul></para> <para>More information about the different stages of a process lifecycle (such as what it means to be placed in the background LRU list) is provided in the  document. </para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Activity Action: Perform assist action. </para> <para>Input: EXTRA_ASSIST_PACKAGE, EXTRA_ASSIST_CONTEXT, can provide additional optional contextual information about where the user was when they requested the assist. Output: nothing. </para>        
				/// </summary>
				/// <java-name>
				/// ACTION_ASSIST
				/// </java-name>
				...
				/// <summary>
				///  <para>An optional field on ACTION_ASSIST and containing additional contextual information supplied by the current foreground app at the time of the assist request. This is a Bundle of additional data. </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Activity Action: Allow the user to select and return one or more existing documents. When invoked, the system will display the various DocumentsProvider instances installed on the device, letting the user interactively navigate through them. These documents include local media, such as photos and video, and documents provided by installed cloud storage providers. </para> <para>Each document is represented as a <c>content:// </c> URI backed by a DocumentsProvider, which can be opened as a stream with ContentResolver#openFileDescriptor(Uri, String), or queried for android.provider.DocumentsContract.Document metadata. </para> <para>All selected documents are returned to the calling application with persistable read and write permission grants. If you want to maintain access to the documents across device reboots, you need to explicitly take the persistable permissions using ContentResolver#takePersistableUriPermission(Uri, int). </para> <para>Callers can restrict document selection to a specific kind of data, such as photos, by setting one or more MIME types in EXTRA_MIME_TYPES. </para> <para>If the caller can handle multiple returned items (the user performing multiple selection), then you can specify EXTRA_ALLOW_MULTIPLE to indicate this. </para> <para>Callers must include CATEGORY_OPENABLE in the Intent so that returned URIs can be opened with ContentResolver#openFileDescriptor(Uri, String). </para> <para>Output: The URI of the item that was picked. This must be a  <c>content:// </c> URI so that any receiver can access it. If multiple documents were selected, they are returned in getClipData().</para> <para> <para>DocumentsContract </para> <para>ACTION_CREATE_DOCUMENT </para> <para>FLAG_GRANT_PERSISTABLE_URI_PERMISSION </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Activity Action: Allow the user to create a new document. When invoked, the system will display the various DocumentsProvider instances installed on the device, letting the user navigate through them. The returned document may be a newly created document with no content, or it may be an existing document with the requested MIME type. </para> <para>Each document is represented as a <c>content:// </c> URI backed by a DocumentsProvider, which can be opened as a stream with ContentResolver#openFileDescriptor(Uri, String), or queried for android.provider.DocumentsContract.Document metadata. </para> <para>Callers must indicate the concrete MIME type of the document being created by setting setType(String). This MIME type cannot be changed after the document is created. </para> <para>Callers can provide an initial display name through EXTRA_TITLE, but the user may change this value before creating the file. </para> <para>Callers must include CATEGORY_OPENABLE in the Intent so that returned URIs can be opened with ContentResolver#openFileDescriptor(Uri, String). </para> <para>Output: The URI of the item that was created. This must be a  <c>content:// </c> URI so that any receiver can access it.</para> <para> <para>DocumentsContract </para> <para>ACTION_OPEN_DOCUMENT </para> <para>FLAG_GRANT_PERSISTABLE_URI_PERMISSION </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Extra used to indicate that an intent can allow the user to select and return multiple items. This is a boolean extra; the default is false. If true, an implementation is allowed to present the user with a UI where they can pick multiple items that are all returned to the caller. When this happens, they should be returned as the getClipData() part of the result Intent.</para> <para> <para>ACTION_GET_CONTENT </para> <para>ACTION_OPEN_DOCUMENT </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Extra used to communicate a set of acceptable MIME types. The type of the extra is <c>String[] </c> . Values may be a combination of concrete MIME types (such as "image/png") and/or partial MIME types (such as "audio/*").</para> <para> <para>ACTION_GET_CONTENT </para> <para>ACTION_OPEN_DOCUMENT </para></para>        
				/// </summary>
				/// <java-name>
				/// EXTRA_MIME_TYPES
				/// </java-name>
				...
				/// <summary>
				///  <para>Optional extra for ACTION_SHUTDOWN that allows the sender to qualify that this shutdown is only for the user space of the system, not a complete shutdown. When this is true, hardware devices can use this information to determine that they shouldn't do a complete shutdown of their device since this is not a complete shutdown down to the kernel, but only user space restarting. The default if not supplied is false. </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>When combined with FLAG_GRANT_READ_URI_PERMISSION and/or FLAG_GRANT_WRITE_URI_PERMISSION, the URI permission grant can be persisted across device reboots until explicitly revoked with Context#revokeUriPermission(Uri, int). This flag only offers the grant for possible persisting; the receiving application must call ContentResolver#takePersistableUriPermission(Uri, int) to actually persist.</para> <para> <para>ContentResolver::takePersistableUriPermission(Uri, int) </para> <para>ContentResolver::releasePersistableUriPermission(Uri, int) </para> <para>ContentResolver::getPersistedUriPermissions() </para> <para>ContentResolver::getOutgoingPersistedUriPermissions() </para></para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>If this is an ordered broadcast, don't allow receivers to abort the broadcast. They can still propagate results through to later receivers, but they can not prevent later receivers from seeing the broadcast. </para>        
				/// </summary>
				/// <java-name>
    ...
    ")V")]
				...
				/// </pre></para> <para> <para>getData </para> <para>setType </para> <para>android.net.Uri::normalizeScheme </para></para>        
				/// </summary>
				/// <returns>
				///  <para>Returns the same Intent object, for chaining multiple calls into a single statement.</para>
				/// </returns>
				...
				/// </pre></para> <para> <para>getType </para> <para>setData </para> <para>normalizeMimeType </para></para>        
				/// </summary>
				/// <returns>
				///  <para>Returns the same Intent object, for chaining multiple calls into a single statement.</para>
				/// </returns>
				...
				/// </pre></para> <para> <para>setType </para> <para>setData </para> <para>setDataAndType </para> <para>normalizeMimeType </para> <para>android.net.Uri::normalizeScheme </para></para>        
				/// </summary>
				/// <returns>
				///  <para>Returns the same Intent object, for chaining multiple calls into a single statement.</para>
				/// </returns>
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>The filter matched an intent with the same data URI scheme and scheme specific part. </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Add a new Intent data "scheme specific part" to match against. The filter must include one or more schemes (via addDataScheme) for the scheme specific part to be considered. If any scheme specific parts are included in the filter, then an Intent's data must match one of them. If no scheme specific parts are included, then only the scheme must match.</para> <para>The "scheme specific part" that this matches against is the string returned by Uri.getSchemeSpecificPart. For Uris that contain a path, this kind of matching is not generally of interest, since addDataAuthority(String, String) and addDataPath(String, int) can provide a better mechanism for matching them. However, for Uris that do not contain a path, the authority and path are empty, so this is the only way to match against the non-scheme part.</para> <para> <para>matchData </para> <para>addDataScheme </para></para>        
				/// </summary>
				/// <java-name>
				/// addDataSchemeSpecificPart
				/// </java-name>
				[Dot42.DexImport("addDataSchemeSpecificPart", "(Ljava/lang/String;I)V", AccessFlags = 17)]
				public void AddDataSchemeSpecificPart(string ssp, int type) /* MethodBuilder.Create */ 
				{
				}

				/// <summary>
				///  <para>Return the number of data scheme specific parts in the filter. </para>        
				/// </summary>
				/// <java-name>
				...
				/// <summary>
				///  <para>Return a data scheme specific part in the filter. </para>        
				/// </summary>
				/// <java-name>
				/// getDataSchemeSpecificPart
				/// </java-name>
				[Dot42.DexImport("getDataSchemeSpecificPart", "(I)Landroid/os/PatternMatcher;", AccessFlags = 17)]
				public global::Android.OS.PatternMatcher GetDataSchemeSpecificPart(int index) /* MethodBuilder.Create */ 
				{
						return default(global::Android.OS.PatternMatcher);
				}

				/// <summary>
				///  <para>Is the given data scheme specific part included in the filter? Note that if the filter does not include any scheme specific parts, false will  <b>always</b> be returned.</para> <para></para>        
				/// </summary>
				/// <returns>
				///  <para>Returns true if the data string matches a scheme specific part listed in the filter. </para>
				/// </returns>
				/// <java-name>
				/// hasDataSchemeSpecificPart
				/// </java-name>
				[Dot42.DexImport("hasDataSchemeSpecificPart", "(Ljava/lang/String;)Z", AccessFlags = 17)]
				public bool HasDataSchemeSpecificPart(string data) /* MethodBuilder.Create */ 
				{
						return default(bool);
				}

				/// <summary>
				///  <para>Return an iterator over the filter's data scheme specific parts. </para>        
				/// </summary>
				/// <java-name>
 ...
 /* scope: __dot42__ */ 
    ...
    "OnFinished;Landroid/os/Handler;Ljava/lang/String;)V", AccessFlags = 1)]
				...
				/// getCreatorPackage
				/// </java-name>
				public virtual string CreatorPackage
				{
				...
				/// </java-name>
				public virtual global::Android.OS.UserHandle CreatorUserHandle
				{
 ...
 /* scope: __dot42__ */ 
    ...
    "g;)V", AccessFlags = 1)]
				...
				/// <summary>
				///  <para>Interface that is implemented to discover when a Loader has been canceled before it finished loading its data. You do not normally need to implement this yourself; it is used in the implementation of android.app.LoaderManager to find out when a Loader it is managing has been canceled so that it can schedule the next Loader. This interface should only be used if a Loader is not being used in conjunction with LoaderManager. </para>    
				/// </summary>
				/// <java-name>
 ...
 /* scope: __dot42__ */ 
				{
						/// <summary>
						///  <para>Called on the thread that created the Loader when the load is canceled.</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// onLoadCanceled
						/// </java-name>
 ...
 /* scope: __dot42__ */ 
		{
				...
				[global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]
				protected internal RestrictionEntry() /* TypeBuilder.AddDefaultConstructor */ 
				{
				}
				...
				/// </java-name>
				public virtual string[] AllSelectedStrings
				{
				...
				/// getSelectedState
				/// </java-name>
				public virtual bool SelectedState
				{
				...
				/// getChoiceValues
				/// </java-name>
				public virtual string[] ChoiceValues
				{
				...
				/// getChoiceEntries
				/// </java-name>
				public virtual string[] ChoiceEntries
				{
 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Builder class for a SyncRequest. As you build your SyncRequest this class will also perform validation. </para>    
				/// </summary>
				/// <java-name>
 ...
 /* scope: __dot42__ */ 
				{
						...
						/// <summary>
						///  <para>Request that a sync occur immediately.</para> <para>Example  <pre>
						...
						/// <summary>
						///  <para>Build a periodic sync. Either this or syncOnce()  <b>must</b> be called for this builder. Syncs are identified by target android.provider/android.accounts.Account and by the contents of the extras bundle. You cannot reuse the same builder for one-time syncs (by calling this function) after having specified a periodic sync. If you do, an  <code>IllegalArgumentException</code> will be thrown.</para> <para>Example usage.</para> <para> <pre>
						///     Request a periodic sync every 5 hours with 20 minutes of flex.
						///     SyncRequest.Builder builder =
						///         (new SyncRequest.Builder()).syncPeriodic(5 * HOUR_IN_SECS, 20 * MIN_IN_SECS);</pre></para> <para> <pre>    Schedule a periodic sync every hour at any point in time during that hour.
						///     SyncRequest.Builder builder =
						///         (new SyncRequest.Builder()).syncPeriodic(1 * HOUR_IN_SECS, 1 * HOUR_IN_SECS);
						/// </pre></para> <para>N.B.: Periodic syncs are not allowed to have any of ContentResolver#SYNC_EXTRAS_DO_NOT_RETRY, ContentResolver#SYNC_EXTRAS_IGNORE_BACKOFF, ContentResolver#SYNC_EXTRAS_IGNORE_SETTINGS, ContentResolver#SYNC_EXTRAS_INITIALIZE, ContentResolver#SYNC_EXTRAS_FORCE, ContentResolver#SYNC_EXTRAS_EXPEDITED, ContentResolver#SYNC_EXTRAS_MANUAL set to true. If any are supplied then an  <code>IllegalArgumentException</code> will be thrown.</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// syncPeriodic
						/// </java-name>
						[Dot42.DexImport("syncPeriodic", "(JJ)Landroid/content/SyncRequest$Builder;", AccessFlags = 1)]
						public virtual Builder SyncPeriodic(long pollFrequency, long beforeSeconds) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para> <para>android.net.ConnectivityManager::isActiveNetworkMetered() </para></para>        
						/// </summary>
						/// <java-name>
						/// setDisallowMetered
						/// </java-name>
						[Dot42.DexImport("setDisallowMetered", "(Z)Landroid/content/SyncRequest$Builder;", AccessFlags = 1)]
						public virtual Builder SetDisallowMetered(bool disallow) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para>Specify an authority and account for this transfer.</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// setSyncAdapter
						/// </java-name>
    ...
    "r;", AccessFlags = 1)]
						public virtual Builder SetSyncAdapter(global::Android.Accounts.Account account, string authority) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para>Optional developer-provided extras handed back in Bundle, String, ContentProviderClient, SyncResult) occurs. This bundle is copied into the SyncRequest returned by build().</para> <para>Example:  <pre>
						///   String[] syncItems = {"dog", "cat", "frog", "child"};
						///   SyncRequest.Builder builder =
						///     new SyncRequest.Builder()
						///       .setSyncAdapter(dummyAccount, dummyProvider)
						///       .syncOnce(5 * MINUTES_IN_SECS);</pre></para> <para> <pre>  for (String syncData : syncItems) {
						///     Bundle extras = new Bundle();
						///     extras.setString("data", syncData);
						///     builder.setExtras(extras);
						///     ContentResolver.sync(builder.build()); // Each sync() request is for a unique sync.
						///   }
						/// </pre> Only values of the following types may be used in the extras bundle:  <ul> <li> <para>Integer </para></li> <li> <para>Long </para></li> <li> <para>Boolean </para></li> <li> <para>Float </para></li> <li> <para>Double </para></li> <li> <para>String </para></li> <li> <para>Account </para></li> <li> <para>null </para></li></ul>If any data is present in the bundle not of this type, build() will throw a runtime exception.</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// setExtras
						/// </java-name>
						...
						/// <summary>
						///  <para>Convenience function for setting ContentResolver#SYNC_EXTRAS_DO_NOT_RETRY.</para> <para>A one-off sync operation that fails will be retried with exponential back-off unless this is set to false. Not valid for periodic sync and will throw an  <code>IllegalArgumentException</code> in build().</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// setNoRetry
						/// </java-name>
						[Dot42.DexImport("setNoRetry", "(Z)Landroid/content/SyncRequest$Builder;", AccessFlags = 1)]
						public virtual Builder SetNoRetry(bool noRetry) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para>Convenience function for setting ContentResolver#SYNC_EXTRAS_IGNORE_SETTINGS.</para> <para>A sync can specify that system sync settings be ignored (user has turned sync off). Not valid for periodic sync and will throw an  <code>IllegalArgumentException</code> in build().</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// setIgnoreSettings
						/// </java-name>
						[Dot42.DexImport("setIgnoreSettings", "(Z)Landroid/content/SyncRequest$Builder;", AccessFlags = 1)]
						public virtual Builder SetIgnoreSettings(bool ignoreSettings) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para>Convenience function for setting ContentResolver#SYNC_EXTRAS_IGNORE_BACKOFF.</para> <para>Force the sync scheduling process to ignore any back-off that was the result of a failed sync, as well as to invalidate any SyncResult#delayUntil value that may have been set by the adapter. Successive failures will not honor this flag. Not valid for periodic sync and will throw an  <code>IllegalArgumentException</code> in build().</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// setIgnoreBackoff
						/// </java-name>
						[Dot42.DexImport("setIgnoreBackoff", "(Z)Landroid/content/SyncRequest$Builder;", AccessFlags = 1)]
						public virtual Builder SetIgnoreBackoff(bool ignoreBackoff) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para>Convenience function for setting ContentResolver#SYNC_EXTRAS_MANUAL.</para> <para>A manual sync is functionally equivalent to calling setIgnoreBackoff(boolean) and setIgnoreSettings(boolean). Not valid for periodic sync and will throw an  <code>IllegalArgumentException</code> in build().</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// setManual
						/// </java-name>
						[Dot42.DexImport("setManual", "(Z)Landroid/content/SyncRequest$Builder;", AccessFlags = 1)]
						public virtual Builder SetManual(bool isManual) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para>An expedited sync runs immediately and will preempt another non-expedited running sync.</para> <para>Not valid for periodic sync and will throw an  <code>IllegalArgumentException</code> in build().</para> <para></para>        
						/// </summary>
						/// <java-name>
						/// setExpedited
						/// </java-name>
						[Dot42.DexImport("setExpedited", "(Z)Landroid/content/SyncRequest$Builder;", AccessFlags = 1)]
						public virtual Builder SetExpedited(bool expedited) /* MethodBuilder.Create */ 
						{
								return default(Builder);
						}

						/// <summary>
						///  <para>Performs validation over the request and throws the runtime exception  <code>IllegalArgumentException</code> if this validation fails.</para> <para></para>        
						/// </summary>
						/// <returns>
						///  <para>a SyncRequest with the information contained within this builder. </para>
						/// </returns>
						/// <java-name>
						/// build
						/// </java-name>
 ...
 /* scope: __dot42__ */ 
		...
		/// <summary>
		///  <para>Description of a single Uri permission grant. This grants may have been created via Intent#FLAG_GRANT_READ_URI_PERMISSION, etc when sending an Intent, or explicitly through Context#grantUriPermission(String, android.net.Uri, int).</para> <para> <para>ContentResolver::getPersistedUriPermissions() </para></para>    
		/// </summary>
		/// <java-name>
 ...
 /* scope: __dot42__ */ 
		{
				/// <summary>
				///  <para>Value returned when a permission has not been persisted. </para>        
				/// </summary>
				/// <java-name>
				/// INVALID_TIME
				/// </java-name>
				...
				/// <summary>
				///  <para>Return the Uri this permission pertains to. </para>        
				/// </summary>
				/// <java-name>
				/// getUri
				/// </java-name>
				...
				/// <summary>
				///  <para>Returns if this permission offers read access. </para>        
				/// </summary>
				/// <java-name>
				/// isReadPermission
				/// </java-name>
				...
				/// <summary>
				///  <para>Returns if this permission offers write access. </para>        
				/// </summary>
				/// <java-name>
				/// isWritePermission
				/// </java-name>
				...
				/// <summary>
				///  <para>Return the time when this permission was first persisted, in milliseconds since January 1, 1970 00:00:00.0 UTC. Returns INVALID_TIME if not persisted.</para> <para> <para>ContentResolver::takePersistableUriPermission(Uri, int) </para> <para>System::currentTimeMillis() </para></para>        
				/// </summary>
				/// <java-name>
				/// getPersistedTime
				/// </java-name>
