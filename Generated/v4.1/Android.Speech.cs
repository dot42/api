 ...
 /* scope: __dot42__ */ 
		{
				...
				/// <summary>
				///  <para>Starts an activity that will prompt the user for speech and send it through a speech recognizer. The results will be returned via activity results (in Activity#onActivityResult, if you start the intent using Activity#startActivityForResult(Intent, int)), or forwarded via a PendingIntent if one is provided.</para> <para>Starting this intent with just Activity#startActivity(Intent) is not supported. You must either use Activity#startActivityForResult(Intent, int), or provide a PendingIntent, to receive recognition results.</para> <para>The implementation of this API is likely to stream audio to remote servers to perform speech recognition which can use a substantial amount of bandwidth.</para> <para>Required extras:  <ul> <li> <para>EXTRA_LANGUAGE_MODEL </para></li></ul></para> <para>Optional extras:  <ul> <li> <para>EXTRA_PROMPT </para></li> <li> <para>EXTRA_LANGUAGE </para></li> <li> <para>EXTRA_MAX_RESULTS </para></li> <li> <para>EXTRA_RESULTS_PENDINGINTENT </para></li> <li> <para>EXTRA_RESULTS_PENDINGINTENT_BUNDLE </para></li></ul></para> <para>Result extras (returned in the result, not to be specified in the request):  <ul> <li> <para>EXTRA_RESULTS </para></li></ul></para> <para>NOTE: There may not be any applications installed to handle this action, so you should make sure to catch ActivityNotFoundException. </para>        
				/// </summary>
				/// <java-name>
